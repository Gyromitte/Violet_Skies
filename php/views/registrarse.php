    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="" method="post">
            ...
            <input class="form-control" type="text" name="nom" placeholder="Nombre" requiered><br><br>
            <label class="form-label" name="ap">Apellido Paterno: </label> 
            <input class="form-control" type="text" name="ap" placeholder="Apellido Paterno" requiered><br><br>
            <label class="form-label" name="am">Apellido Materno: </label> 
            <input class="form-control" type="text" name="am" placeholder="Apellido Materno" requiered><br><br>
            <label class="form-label" name="usu">Correo: </label> 
            <input class="form-control" type="email" name="usu" placeholder="Correo" requiered><br><br>
            <label class="form-label" name="cel">Numero de Celular: </label> 
            <input class="form-control" type="text" name="cel" placeholder="Telefono" requiered><br><br>
            ...
            <div id="mensajeEmpleado" class="alert" style="display: none;"></div>
        </form>
      </div>
    </div>
  </div>
...
data-bs-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="" method="post">
            ...
            <input class="form-control" type="text" name="cel" placeholder="Telefono" requiered><br><br>
            <label class="form-label" name="pass">Contraseña: </label>
            <input class="form-control" type="password" name="pass" placeholder="Contraseña" requiered><br><br>
            <label class="form-label" name="ckpass">Comprobar Contraseña: </label>
            <input class="form-control" type="password" name="ckpass" placeholder="Comprobar Contraseña" requiered><br><br>
            <button class="loginButton" type="submit" name="regc">Registrarse</button>   
        </form>
   ...
   <!-- Add this script at the end of the body tag or in a separate JS file -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    // Handler for Empleado registration form submission
    $("#registroEmpleadoForm").on("submit", function (event) {
        event.preventDefault(); // Prevent form submission

        // Perform AJAX request to handle registration
        $.ajax({
            type: "POST",
            url: "../scripts/registro_emp.php", // Replace with the PHP script handling Empleado registration
            data: $(this).serialize(),
            success: function (response) {
                if (response === "success") {
                    // Show success message in the modal
                    $("#mensajeEmpleado").html("<p>Registro exitoso como Empleado.</p>").show();
                } else {
                    // Show error message in the modal
                    $("#mensajeEmpleado").html("<p>Error en el registro. Verifica los campos.</p>").show();
                }
            }
        });
    });

    // Handler for Cliente registration form submission
    $("#registroClienteForm").on("submit", function (event) {
        event.preventDefault(); // Prevent form submission

        // Perform AJAX request to handle registration
        $.ajax({
            type: "POST",
            url: "register_cliente.php", // Replace with the PHP script handling Cliente registration
            data: $(this).serialize(),
            success: function (response) {
                if (response === "success") {
                    // Show success message in the modal
                    $("#mensajeCliente").html("<p>Registro exitoso como Cliente.</p>").show();
                } else {
                    // Show error message in the modal
                    $("#mensajeCliente").html("<p>Error en el registro. Verifica los campos.</p>").show();
                }
            }
        });
    });
